...
object ItemUtils {
  ...
  def addStackToSlots(
    ...
    for (
      slot <- slots
      if (!checkValid || inv.isItemValidForSlot(slot, stack)) && isSameItem(
        stack,
      ...
      val toAdd = Misc.min(
        target.getMaxStackSize - target.stackSize,
        inv.getInventoryStackLimit - target.stackSize,
        stack.stackSize
      )
  ...
  def findItemInInventory(inv: IInventory, item: Item) =
    Range(0, inv.getSizeInventory)
      .map(x => x -> inv.getStackInSlot(x))

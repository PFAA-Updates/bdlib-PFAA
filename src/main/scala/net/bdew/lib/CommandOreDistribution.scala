...
object CommandOreDistribution extends CommandBase {
  ...
  def safeInt(s: String) =
    try {
      ...
      case e: NumberFormatException =>
        throw new WrongUsageException("bdlib.oredistribution.usage")
    }

  def wrapColor(s: String, c: EnumChatFormatting) =
    c + s + EnumChatFormatting.RESET
  ...
  def processCommand(sender: ICommandSender, baseParams: Array[String]) {
    ...
    val maxY =
      if (params.length >= 3) safeInt(params(2)) else world.getHeight - 1
    ...
    val ores = (OreDictionary.getOreNames.filter(x =>
      x.startsWith("ore") || x.startsWith("denseore")
    ) flatMap { name =>
      val fixedName =
      ...
      for (s <- OreDictionary.getOres(name))
        yield (s.getItem, s.getItemDamage) -> fixedName
    }).toMap

    val distribution = mutable.Map.empty[String, Int].withDefaultValue(0)
    val kinds =
      mutable.Map.empty[String, Set[(Item, Int)]].withDefaultValue(Set.empty)
    ...
    for {
      ...
      val key = (
        Item.getItemFromBlock(world.getBlock(x, y, z)),
    ...
    if (toFile) {
      ...
      try {
        dumpWriter.write(
          "Ore distribution report - (%d,%d,%d) to (%d,%d,%d) dim %d".format(
            startX - radius,
            minY,
            startZ - radius,
            startX + radius,
        ...
        if (warnings.nonEmpty) {
          for ((id, types) <- warnings.toList.sortBy(_._1)) {
            dumpWriter.write(
              "Warning: %s has multiple variants generated:".format(id)
            )
            dumpWriter.newLine()
            for ((item, dmg) <- types) {
              dumpWriter.write(
      ...
      for ((id, types) <- warnings.toList.sortBy(_._1)) {
        CommandBase.func_152373_a(
          sender,
          this,
          "bdlib.oredistribution.warn",
          wrapColor(" !", EnumChatFormatting.RED),
          wrapColor(id, EnumChatFormatting.RED)
        )
        for ((item, dmg) <- types)
          CommandBase.func_152373_a(
            sender,
            this,
            wrapColor(" - ", EnumChatFormatting.RED) + GameData.getItemRegistry
              .getNameForObject(item) + "@" + dmg
          )
      }
      for ((id, num) <- distribution.filter(_._2 > 0).toList.sortBy(-_._2)) {
        CommandBase.func_152373_a(

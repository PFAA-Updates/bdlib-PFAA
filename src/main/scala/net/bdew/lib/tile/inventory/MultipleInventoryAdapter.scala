...
class MultipleInventoryAdapter(val map: Map[Int, (IInventory, Int)])
    extends IInventory {
  def this(inventories: Iterable[IInventory]) =
    this((inventories flatMap { inv =>
      (0 until inv.getSizeInventory).map((inv, _))
  ...
  override lazy val getInventoryStackLimit =
    inventories.map(_.getInventoryStackLimit).min
  ...
  override def isUseableByPlayer(p: EntityPlayer) =
    inventories.exists(_.isUseableByPlayer(p))

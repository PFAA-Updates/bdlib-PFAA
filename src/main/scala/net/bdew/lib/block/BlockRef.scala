...
case class BlockRef(x: Int, y: Int, z: Int) {
  def isValid(w: IBlockAccess) =
    y >= 0 && y < 256 // hardcoded because getHeight is unavailable on servers :(

  def block(w: IBlockAccess) =
    if (isValid(w)) Option(w.getBlock(x, y, z)) else None
  def tile(w: IBlockAccess) =
    if (isValid(w)) Option(w.getTileEntity(x, y, z)) else None
  ...
  def neighbour(side: ForgeDirection) =
    BlockRef(x + side.offsetX, y + side.offsetY, z + side.offsetZ)
  def neighbours =
    ForgeDirection.VALID_DIRECTIONS.map(x => x -> neighbour(x)).toMap

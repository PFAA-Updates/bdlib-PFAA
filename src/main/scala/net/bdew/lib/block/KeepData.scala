...
/** Mixin for blocks to keep their data when broken Must have a TileEntity that
  * extends TileKeepData
  */
trait BlockKeepData extends Block with HasItemBlock {
  this: HasTE[_ <: TileKeepData] =>

  override val ItemBlockClass: Class[_ <: ItemBlockKeepData] =
    classOf[ItemBlockKeepData]

  def getSavedBlock(world: World, x: Int, y: Int, z: Int, metadata: Int) = {
    val stack = new ItemStack(
      getItemDropped(metadata, world.rand, 0),
      1,
  ...
  /** Override for special actions on placing ItemStack might or might not
    * contain actual data Called with the TE created and added to world, after
    * onBlockPlacedBy & co
    */
  def restoreTileEntity(
      world: World,
      x: Int,
      y: Int,
      z: Int,
      is: ItemStack,
      player: EntityPlayer
  ) = getTE(world, x, y, z).loadFromItem(is)
}

/** Mixin for tile entities to keep their data when broken Block class has to
  * extend BlockKeepData
  */
trait TileKeepData extends TileExtended {

  /** Override to modify data that will be stored in an item
    * @param t
    *   Automatically serialized data
    * @return
    *   Modified serialized data
    */
  def afterTileBreakSave(t: NBTTagCompound) = t

  /** Override to modify data that will be load from an item After the tile
    * entity is fully constructed and added to world
    * @param t
    *   Automatically serialized data
    * @return
    *   Modified serialized data
  ...
  final def saveToItem(is: ItemStack): Unit = {
    ...
    is.getTagCompound.setTag(
      "data",
      afterTileBreakSave(NBT.from(persistSave.trigger))
    )
  }
  ...
  final def loadFromItem(is: ItemStack): Unit = {
    if (is.hasTagCompound && is.getTagCompound.hasKey("data")) {
      persistLoad.trigger(
        beforeTileBreakLoad(is.getTagCompound.getCompoundTag("data"))
      )
    }
  }
}
  ...
  override def placeBlockAt(
    ...
    if (
      ...
      if (!world.isRemote && b.isInstanceOf[BlockKeepData])
        b.asInstanceOf[BlockKeepData]
          .restoreTileEntity(world, x, y, z, stack, player)
      true
    } else false
  }
}

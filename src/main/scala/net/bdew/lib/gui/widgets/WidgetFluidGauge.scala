...
class WidgetFluidGauge(
    val rect: Rect,
    overlay: Texture,
    dSlot: DataSlotTankBase
) extends Widget {
  override def handleTooltip(p: Point, tip: mutable.MutableList[String]) {
    if (dSlot.getFluid != null) {
      tip += dSlot.getFluid.getFluid.getLocalizedName(dSlot.getFluid)
      tip += "%s/%s mB".format(
        DecFormat.round(dSlot.getFluidAmount),
        DecFormat.round(dSlot.getCapacity)
      )
    } else {
  ...
  override def draw(mouse: Point) {
    ...
    if (fStack != null) {
      ...
      var fillHeight =
        if (dSlot.getCapacity > 0) rect.h * fStack.amount / dSlot.getCapacity
        else 0
      var yStart: Int = 0
      ...
      while (fillHeight > 0) {
        if (fillHeight > 16) {
          parent.drawTexture(
            new Rect(rect.x, rect.y2 - 16 - yStart, rect.w, 16),
            icon,
            color
          )
          fillHeight -= 16
        } else {
          parent.drawTextureInterpolate(
            new Rect(rect.x, rect.y2 - 16 - yStart, rect.w, 16),
            icon,
            0,
            1 - fillHeight / 16,

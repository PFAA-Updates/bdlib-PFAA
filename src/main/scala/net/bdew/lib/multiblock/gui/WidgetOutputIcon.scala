...
class WidgetOutputIcon(p: Point, te: CIOutputFaces, output: Int)
    extends Widget {
  ...
  override def draw(mouse: Point) {
    ...
    if (faces.isDefinedAt(output)) {
      ...
      parent.drawTexture(
        rect,
        Texture(Texture.BLOCKS, te.resources.disabled),
        te.resources.outputColors(output)
      )
    }
  }
    ...
    if (faces.isDefinedAt(output)) {
      ...
      bf.origin.block(te.getWorldObj) map { block =>
        ...
        tip += "%d, %d, %d - %s".format(
          ...
          bf.z,
          Misc.toLocal(
            "bdlib.multiblock.face." + bf.face.toString.toLowerCase(Locale.US)
          )
        )
      }
    } else {

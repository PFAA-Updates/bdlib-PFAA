...
abstract class BlockController[T <: TileController](
    val name: String,
    material: Material,
    val TEClass: Class[T]
  ...
  override def getTooltip(
    ...
    List(Misc.toLocal("bdlib.multiblock.tip.controller")) ++ (for (
      (kind, max) <- machine.modules
    ) yield {
      ...
      if (machine.required.getOrElse(kind, 0) > 0) {
        Misc.toLocalF(
          ...
          EnumChatFormatting.YELLOW + machine
            .required(kind)

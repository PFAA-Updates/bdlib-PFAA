...
case class DataSlotSensor[T, R](
    registry: SensorSystem[T, R],
    name: String,
    parent: TileRedstoneSensorModule,
    default: GenericSensorType[T, R]
) extends DataSlotVal[SensorPair[T, R]] {
  ...
  override def load(t: NBTTagCompound, kind: UpdateKind.Value) = {
    value = registry
      .get(t.getCompoundTag(name).getString("uid"))

...
trait GenericConfigLoader extends RecipeLoader {
  ...
  def processConfigStatement(section: ConfigSection, s: CfgEntry): Unit =
    s match {
      case CfgVal(id, v) =>
        BdLib.logDebug("Config: %s%s = %s", section.pfx, id, v)
        section.set(id, v)
      case CfgSection(id, st) =>
        processConfigBlock(section.getOrAddSection(id), st)
      case x =>
  ...
  override def processConfigStatement(s: ConfigStatement) = s match {
    case CsCfgSection(id, st) =>
      processConfigBlock(cfgStore.getOrAddSection(id), st)

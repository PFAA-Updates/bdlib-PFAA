...
/** Main recipe loader class The input file is parsed by
  * [[net.bdew.lib.recipes.RecipeParser]] Into a list of
  * [[net.bdew.lib.recipes.ConfigStatement]] subclasses That are then executed
  * here
  */
...
class RecipeLoader {
  ...
  /** Create a new parser, override in subclasses to use an extended parser
    * @return
    *   New ConfigParser (or subclass) instance
    */
  ...
  var currCharMap = Map
    .empty[Char, StackRef]
    .withDefault(x => error("Undefined recipe character '%s'", x))

  /** Current map of class macros
    */
  var currClassMacros = Map
    .empty[String, String]
    .withDefault(x => error("Undefined class macro '%s'", x))

  /** Triggers an error with formatted string
    */
  def error(msg: String, params: Any*) = throw new StatementError(
    msg.format(params: _*)
  )
  ...
  /** Looks up a recipe component
    * @param s
    *   Parser ItemStack reference
    * @return
    *   ItemStack or String that can be used as recipe components in MC
    *   functions
    */
  ...
  /** Sanitize items from reflection
    ...
    *   The item
    * @param source
    *   Human readable source (used in errors and warnings)
    * @param meta
    *   Metadata or damage
    * @param cnt
    *   Stack size
  ...
  def sanitizeReflectedItem(
    ...
    case x: ItemStack =>
      ...
      if (meta != OreDictionary.WILDCARD_VALUE && meta != l.getItemDamage) {
        BdLib.logWarn(
          "%s requested with meta %d but specifies %d",
          source,
          meta,
          l.getItemDamage
        )
    ...
    case _ =>
      error(
        "%s is %s which cannot be translated to an ItemStack",
        source,
        x.getClass.getName
      )
  }

  /** Fetches an ItemStack using reflection from static fields Item and Block
    * instances are converted to ItemStacks
    * @param clsName
    *   Name of class to fetch from (can be a class macro)
    * @param fldName
    *   Field name
    * @param meta
    *   Metadata or damage
  ...
  def reflectStack(
    ...
    return sanitizeReflectedItem(
      fld,
      "%s.%s".format(realName, fldName),
      meta,
      cnt
    )
  }

  /** Fetches an ItemStack using reflection from static getters Item and Block
    * instances are converted to ItemStacks
    * @param clsName
    *   Name of class to fetch from (can be a class macro)
    * @param method
  ...
  def reflectStackGetter(
    ...
    val res = Class
      .forName(realName)
      .getMethod(realMethod, classOf[String])
      .invoke(null, param)
    return sanitizeReflectedItem(
      res,
      "%s.%s(%s)".format(realName, realMethod, param),
      meta,
      cnt
    )
  }

  /** Returns all possible ItemStacks that match a reference Currently
    * everything except OreDictionary references just returns one item
    * @param s
    *   Parser ItemStack reference
    * @param cnt
    *   Stack size
    ...
    */
  def getAllConcreteStacks(s: StackRef, cnt: Int = 1): Iterable[ItemStack] =
    s match {
  ...
  /** Returns an ItemStack that match a reference This is the main StackRef
    * resolution method
    * @param s
    *   Parser ItemStack reference
    * @param cnt
  ...
  def getConcreteStack(s: StackRef, cnt: Int = 1): ItemStack = s match {
    case StackOreDict(id) =>
      ...
      if (l.size == 0)
        error(
          "Concrete ItemStack requested for OD entry '%s' that is empty",
          id
        )
      val s = l.get(0).copy()
      s.stackSize = cnt
    ...
    case StackGeneric(mod, id) =>
      notNull(
        GameRegistry.findItemStack(mod, id, cnt),
        "Stack not found %s:%s".format(mod, id)
      )
    case StackBlock(mod, id, meta) =>
      val block = notNull(
        GameRegistry.findBlock(mod, id),
        "Block not found %s:%s".format(mod, id)
      )
      if (Item.getItemFromBlock(block) == null)
        error(
          "The block '%s:%s' does not have a valid matching item registered and thus can't be used in recipes",
          mod,
          id
        )
    ...
    case StackItem(mod, id, meta) =>
      new ItemStack(
        notNull(
          GameRegistry.findItem(mod, id),
          "Item not found %s:%s".format(mod, id)
        ),
        cnt,
        meta
      )
    case StackReflect(cls, field, meta) => reflectStack(cls, field, meta, cnt)
    case StackGetter(cls, method, param, meta) =>
      reflectStackGetter(cls, method, param, meta, cnt)
  }
  ...
  /** Looks up all characters used in the recipe
    ...
    *   The pattern
    * @return
    *   Map from character to recipe components and a boolean that means
    *   OD-aware methods should be used
    */
  def resolveRecipeComponents(
      s: Iterable[Char]
  ): (Map[Char, AnyRef], Boolean) = {
  ...
  /** Helper to remove recipes from lists
    ...
    *   list to check
    * @param res
    *   crafting result to remove
    * @return
    *   cleaned list
    */
  def clearStatements(
      list: List[RecipeStatement],
      res: StackRef
  ): List[RecipeStatement] = {
    list flatMap {
      ...
      case RsConditional(cnd, thn, els) =>
        Some(
          RsConditional(
            cnd,
            clearStatements(thn, res),
            clearStatements(els, res)
          )
        )
      case x => Some(x)
    }
  ...
  def processRecipeStatement(st: RecipeStatement) = st match {
    ...
    case RsRecipeSmelting(in, out, cnt, xp) =>
      BdLib.logDebug(
        "Adding smelting recipe %s => %s * %d (%f xp)",
        in,
        out,
        cnt,
        xp
  ...
  /** Processes recipe statements in a new context Any changes to mutable state
    * will not persist when this method returns
    */

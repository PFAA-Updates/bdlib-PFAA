...
object ConnectedHelper {
  ...
  val neighbourFaces = (for (x <- ForgeDirection.VALID_DIRECTIONS)
    yield x -> new neighbourFaces(x)).toMap

  // Maps edge number + face to 3d vertices
  val draw =
    (for (
      (rect, n) <- faceEdges.zipWithIndex;
  ...
  case class Vec3F(x: Float, y: Float, z: Float) {
    ...
    def -(m: Vec3F) = Vec3F(x - m.x, y - m.y, z - m.z)
    def +(d: ForgeDirection) =
      Vec3F(x + d.offsetX, y + d.offsetY, z + d.offsetZ)
  ...
  case class RectF(x1: Float, y1: Float, x2: Float, y2: Float) {
    def corners =
      Seq(Vec2F(x1, y1), Vec2F(x2, y1), Vec2F(x2, y2), Vec2F(x1, y2))
  }
  ...
  def mapToIcon(v: Vec2F, i: IIcon) =
    Vec2F(i.getInterpolatedU(v.u * 16), i.getInterpolatedV((1 - v.v) * 16))
